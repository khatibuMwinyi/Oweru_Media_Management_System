...
<head>
    ...
    <style>
        ...
        /* Category backgrounds matching PostCard exactly */
        .bg-rentals, .bg-lands_and_plots { 
            background: #C89128; 
        }
        .bg-property_sales, .bg-property_services { 
            background: #d1d5db; 
        }
        ...
        .text-property_sales, .text-property_services { 
            color: #374151; 
        }
    ...
    <div class="posts-container">
        <div class="post-grid">
            ${filteredPosts.map((post) => {
              const categoryClass = post.category || 'default';
              const bgClass = `bg-${categoryClass}`;
              const textClass = `text-${categoryClass}`;
              return `
                <div class="post-card ${bgClass}">
                    <div class="post-media">
                        ${post.post_type === "Reel" && post.media?.filter((m) => m.file_type === "video").length > 0
                            ...
                            : '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:white;">No media</div>'}
                    </div>
                    ${post.post_type !== "Reel" ? `
                        ...
                        <div class="px-4 pt-4 pb-3">
                            <div class="flex justify-between items-start">
                                <div class="flex-1">
                                    ...
                                    <div class="post-meta ${textClass} mt-2 text-left">${post.post_type || 'Unknown'} • ${post.category || 'Uncategorized'} • ${new Date(post.created_at).toLocaleDateString()}</div>
                                </div>
                            </div>
                        </div>
                        <div class="px-4 py-4 h-32 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 flex-shrink-0">
                            <div class="${textClass} text-left whitespace-pre-wrap text-sm leading-relaxed">${post.description || 'No description available.'}</div>
                        </div>
                        <div class="px-4 pb-3 flex justify-end items-center">
                            <div style="font-size: 12px; color: #6b7280;">Oweru Media</div>
                        </div>
                    </div>
                    ` : ''}
                </div>
  ...
  const handleCategoryFilter = (categoryId) => {
    ...
    if (category) {
      ...
      setTimeout(() => {
        ...
        if (postsSection) {
          window.scrollTo({ top: postsSection.offsetTop - 100, behavior: "smooth" });
        }
      }, 100);
    }
  };
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-16 lg:py-24">
            ...
            <p className="text-xl sm:text-2xl text-gray-200 mb-10 max-w-3xl mx-auto leading-relaxed font-medium">
              Streamline your real estate content with professional post management,
              multi-format media support, and intelligent categorization
            </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex items-center gap-3">
                <div className="flex bg-gray-100 rounded-lg p-1">
                  <button
                    ...
                    className={`px-3 py-2 rounded-md transition-colors ${viewMode === "grid" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}
                    title="Grid View"
                  >
                  ...
                  <button
                    ...
                    className={`px-3 py-2 rounded-md transition-colors ${viewMode === "list" ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"}`}
                    title="List View"
                  >
                ...
                >
                  {downloadingAll ? (
                    ...
                    <><Download className="w-4 h-4" /><span>Download All</span></>
                  )}
                </button>
              </div>
            </div>
          ...
          {!loading && !error && filteredPosts.length === 0 && (
            <div className="text-center py-20 bg-white rounded-xl shadow-md border border-gray-200">
              ...
              {(selectedCategory !== "all" || selectedPostType !== "all" || searchQuery) && (
                <button
                  onClick={() => { setSearchQuery(""); setSelectedCategory("all"); setSelectedPostType("all"); }}
          ...
          {!loading && !error && filteredPosts.length > 0 && (
            <>
              ...
              {/* List view — single column, card only (no duplicate content) */}
              {viewMode === "list" && (
      ...
      <section className="py-20 bg-gradient-to-r from-[#C89128] to-[#B08020]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-white/90 mb-8 font-medium">
            Join our team of administrators and moderators to create and manage
